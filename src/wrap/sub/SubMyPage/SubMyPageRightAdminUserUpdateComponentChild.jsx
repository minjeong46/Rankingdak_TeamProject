...
export default function SubMyPageRightAdminUserUpdateComponentChild ({회원정보}) {
    ...
    const onClickIdCheck=(e)=>{
        ...
        if(regExp2.test(value)===false || regExp3.test(value)===true || regExp4.test(value)===true || regExp5.test(value)===false || regExp6.test(value)===false){
            ...
            axios({
                url:'https://mj46.co.kr/rankingdak/rankingdak_id_check.php',
                method:'POST',
                data: formDate
            })
            .then((res)=>{
    ...
    React.useEffect(()=>{
        ...
        if(state.비밀번호!=='' && state.비밀번호 === state.비밀번호확인 && state.이름.length>2 && state.phoneSuccessCheck === true && state.이메일!=='' && state.평생회원 !== ''){
            isUpdateBtn = true;
        }
        else {
            isUpdateBtn = false;
                                    ...
                                    <div className="input-btn-box">
                                        ...
                                        <label htmlFor="userLiftMemberAgree">
                                            <input
                                                ...
                                                checked={state.평생회원.includes('동의')}
                                            />
                                            <span>동의</span>
                                        </label>
                                        <label htmlFor="userLiftMemberAgreeNone">
                                            <input
                                                ...
                                                value={'동의안함'}
                                                onChange={onChangeMember}
                                                checked={state.평생회원.includes('동의안함')}
                                            />
                                            <span>동의안함</span>
                                        </label>
                                    </div>
