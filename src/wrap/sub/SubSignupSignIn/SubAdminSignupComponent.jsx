...
export default function SubAdminSignupComponent () {
    ...
    React.useEffect(()=>{
        ...
        if(state.아이디중복확인===true && state.비밀번호!=='' && state.비밀번호 === state.비밀번호확인 && state.이름.length>1 && state.phoneSuccessCheck === true && state.이메일!==''){
            isSignupBtn = true;
        }
        else {
            isSignupBtn = false;
    ...
    const onSubmitSignUp=(e)=>{
        ...
        formData.append('adminLiftMember', state.평생회원);
        ...
        .then((res)=>{
            if(res.status===200){
                console.log(res.data); 
                if(res.data===1){
                    confirmModalMethod('관리자님 회원가입을 축하드립니다!');
