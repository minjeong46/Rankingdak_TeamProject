...
export default function SubAdminLoginComponent () {
    ...
    React.useEffect(()=>{
        if(localStorage.getItem('RANKING_USER_ID_AUTO')!==null){
            if(localStorage.getItem('RANKING_ADMIN_ID_AUTO')!==''){
                setState({
                    ...state,
                    아이디: JSON.parse(localStorage.getItem('RANKING_ADMIN_ID_AUTO'))
                })
            }
            else {
                setState({
    ...
    const onClickIdSearch=(e)=>{
        e.preventDefault();
        navigate('/adminSigninIdSearch');
    }
    const onClickPwSearch=(e)=>{
        e.preventDefault();
        navigate('/adminSigninPwSearch');
    }
    ...
    const onSubmitLogin=(e)=>{
        ...
        axios({
            url: 'https://mj46.co.kr/rankingdak/rankingdak_admin_signin.php',
            method: 'POST',
            data: formData,
        })
        .then((res)=>{
            ...
            if(res.status===200){
                if(res.data===0){
                    ...
                    const 로그인정보 = {
                        ...
                        아이디저장: `${state.아이디저장===true?
                                    localStorage.setItem('RANKING_ADMIN_ID_AUTO',JSON.stringify(res.data.아이디)):res.data.아이디}`,
                        
                    }
                    localStorage.setItem('RANKING_SIGNIN_DATA', JSON.stringify(로그인정보));

                    dispatch(signIn(로그인정보));
                    confirmModalMethod('관리자님, 랭킹닭컴에 오신 것을 환영합니다.');
