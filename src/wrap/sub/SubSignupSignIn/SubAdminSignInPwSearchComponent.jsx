...
export default function SubAdminSignInPwSearchComponent () {
    ...
    const onSubmitPhone=(e)=>{
        ...
        axios({
            url:'https://mj46.co.kr/rankingdak/rankingdak_admin_pw_search_phone.php',
            method:'POST',
            data: formData 
        })
        .then((res)=>{
            if(res.status===200){
                if(res.data===0){
                    confirmModalMethod('휴대폰 번호를 확인하고 다시 시도해주세요');   
                }
                else {
                    confirmModalMethod('비밀번호를 재설정하세요');

                        navigate('/adminSigninPwReset', {
                            state : {
                                아이디: state.아이디
                            }
                        })
    ...
    const onSubmitEmail=(e)=>{
        ...
        axios({
            url:'https://mj46.co.kr/rankingdak/rankingdak_admin_pw_search_email.php',
            method:'POST',
            data: formData 
        })
        .then((res)=>{
            if(res.status===200){
                if(res.data===0){
                    confirmModalMethod('이메일을 확인하고 다시 시도해주세요');   
                }
                else {
                    confirmModalMethod('비밀번호를 재설정하세요');
    ...
    return (
        <main id='signin-admin-pw-search' className='signin-search'>
            <section id="section1">
                <div className="container">
                    <div className="title">
                        <h2><em>관리자</em><br/> 비밀번호찾기</h2>
                    </div>
