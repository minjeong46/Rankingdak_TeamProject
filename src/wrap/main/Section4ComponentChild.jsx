...
export default function Section4ComponentChild({product, n, timeSale}){
    ...
    const refSlideWrap = React.useRef(<ul></ul>);
    ...
    const mainSlide=()=>{
        refSlideWrap.current.style.transition = `all 0.4s ease-in-out`;
        refSlideWrap.current.style.left = `${-760 * cnt}px`;
        if(cnt!==0){
            returnSlide();
        }
    }
    ...
    const returnSlide=()=>{
        if(cnt>n/2-1){
            ...
            setToggle(1);
            refSlideWrap.current.style.transition = `all 0.4s ease-in-out`;
            refSlideWrap.current.style.left  = `${-380 * 0}px`
        }
        else if(cnt<0){
            ...
            refSlideWrap.current.style.left = `${-380 * n}px`
        }
    }
    ...
    const onClickViewProduct=(e,item,path)=>{
        ...
        let obj = {
            ...
            제품명: item.상품명,
            판매가: (Math.round(item.가격*(1-item.할인율)/100)*100),할인율: item.할인율,
            정가: item.가격,
            별점: item.별점,
            리뷰: item.토탈,
            개별가격: item.개별가격,
                ...
                <div className="content">
                    <div className="slide-container" onMouseEnter={onMouseEnterSlideContainer} onMouseLeave={onMouseLeaveSlideContainer}>
                        ...
                        <div className="prev-allow-btn-box">
                            <button className={`prev-allow-btn${cnt===0 ? ' on' : ''}`} onClick={onClickPrevBtn}></button>
                        </div>
                        <div className="next-allow-btn-box">
                            <button className={`next-allow-btn${cnt===(n/2)-1 ? ' on' : ''}`} onClick={onClickNextBtn}></button>
                        </div>
                    </div>
                </div>
            </div>
